...
public class BoardDao {
  ...
  public List<BoardVo> bList(int startNo, int pageSize) {
    ...
    try {
      sql = "select * from board2 order by idx desc limit ?, ?";
  ...
  private int replyCount(int idx) {
    try {
      sql = "select count(*) from board2_cont where board2_idx = ?";
  ...
  public int bInputOk(BoardVo vo) {
    try {
      sql = "insert into board2 values (default,?,?,?,?,?,default,0,?,0,?)";
      pstmt = conn.prepareStatement(sql);
  ...
  public BoardVo bContent(int idx) {
    ...
    try {
      sql = "select * from board2 where idx = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      rs = pstmt.executeQuery();
      rs.next();
  ...
  public void readNumUpdate(int idx) {
    try {
      sql = "update board2 set readnum = readnum + 1 where idx = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      pstmt.executeUpdate();
    } catch (SQLException e) {
  ...
  public int bDelete(int idx, String pwd) {
    ...
    try {
      sql = "delete from board2 where idx = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      pstmt.executeUpdate();
  ...
  public int getSearch(int idx, String pwd) {
    try {
      sql = "select idx from board2 where idx = ? and pwd = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
  ...
  public int bUpdateOk(BoardVo vo, int idx) {
    try {
      sql = "update board2 set name=?,title=?,email=?,homepage=?,pwd=?,hostip=?,content=? where idx=?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setString(1, vo.getName());
      pstmt.setString(2, vo.getTitle());
      pstmt.setString(3, vo.getEmail());
  ...
  public void bGood(int idx) {
    try {
      sql = "update board2 set good = good + 1 where idx = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      pstmt.executeUpdate();
  ...
  public void bContInput(BoardContVo cVo) {
    try {
      sql = "insert into board2_cont values (default,?,?,?,default,?,?)";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, cVo.getBoard2_idx());
  ...
  public List<BoardContVo> contList(int idx) {
    ...
    try {
      sql = "select * from board2_cont where board2_idx=? order by idx";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      rs = pstmt.executeQuery();
      ...
      while(rs.next()) {
        ...
        cVo.setBoard2_idx(rs.getInt("board2_idx"));
  ...
  public void replyDel(int idx) {
    try {
      sql = "delete from board2_cont where idx = ?";
      pstmt = conn.prepareStatement(sql);
      pstmt.setInt(1, idx);
      pstmt.executeUpdate();
    } catch (SQLException e) {
  ...
  public List<BoardVo> getBSearch(String search, String searchString) {
    ...
    try {
      sql = "select * from board2 where " + search + " like ? order by idx desc";
      pstmt = conn.prepareStatement(sql);
  ...
  public int totSearchRecCnt(String search, String searchString) {
    try {
      sql = "select count(*) from board2 where " + search + " like ? order by idx desc";
      pstmt = conn.prepareStatement(sql);
      pstmt.setString(1, "%"+searchString+"%");
      rs = pstmt.executeQuery();
      rs.next();
