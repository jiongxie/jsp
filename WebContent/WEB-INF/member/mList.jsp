...
<head>
  ...
  <link href="./allweather/assets/apple-touch-icon.png" rel="apple-touch-icon">
  <link href="./allweather/assets/favicon.ico" rel="icon">

  <title>awmList.jsp</title>  
            ...
            <div class="col-xs-12">
                ...
                <div class="container">
								...
								<%
								    ...
								    int curScrNo = (int) request.getAttribute("curScrNo");
								...
								%>
								      <tr>
								        ...
								        <td><% if(vo.getUserinfor().equals("공개")) { %><%=vo.getName()%><% } else { %>비공개<% } %></td>
								        <td><% if(vo.getUserinfor().equals("공개")) { %><%=vo.getGender()%><% } else { %>비공개<% } %></td>
								        <td><% if(vo.getUserinfor().equals("공개")) { %><%=vo.getBirthday().substring(0, 10)%><% } else { %>비공개<% } %></td>
								        <td><% if(vo.getUserinfor().equals("공개")) { %><%=vo.getEmail()%><% } else { %>비공개<% } %></td>
								        <td><% if(vo.getUserinfor().equals("공개")) { %><%=vo.getHobby()%><% } else { %>비공개<% } %></td>
								        <td><%=strlevel%></td>
								      </tr>
								<%
								      curScrNo--;
								...
								%>    
								  ...
								  <div style="text-align:center;">
								    <% if(pag != 1) { %>
								       [<a href="mList.mem?pag=1" style="color:blue;">첫페이지</a>]
								    <% } %>
								    <% if(pag > 1) { %>
								       [<a href="mList.mem?pag=<%=pag-1%>" style="color:blue;">이전페이지</a>]
								    <% } %>
								    <%=pag%>Page / <%=totPage%>Pages 
								    <% if(pag < totPage) { %>
								       [<a href="mList.mem?pag=<%=pag+1%>" style="color:blue;">다음페이지</a>]
								    <% } %>
								    <% if(pag != totPage) { %>
								       [<a href="mList.mem?pag=<%=totPage%>" style="color:blue;">끝페이지</a>]
								    <% } %>
								  </div>
								<!-- 페이징 처리 끝   -->  
								</div>
                <h1><a href="./allweather/allmain.html" style="color:black;">All weather investment</a></h1>
                <p><br/><p>
