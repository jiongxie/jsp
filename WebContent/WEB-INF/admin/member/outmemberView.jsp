...
<%
  request.setCharacterEncoding("utf-8");
	...
	ArrayList<MemberVo> ovos = (ArrayList<MemberVo>) request.getAttribute("ovos");
%>
<!DOCTYPE html>
...
<link href="./main.97292821.css" rel="stylesheet">
    ...
    .bGrid {border:1px solid #555; padding:5px 0px 5px 10px}
  </style>
</head>
<body>
            ...
            <div class="col-xs-12">
                ...
                <div class="container">
								  <form>
								    ...
								    <div class="form-group bGrid">
								    <% for(int i=0; i<ovos.size(); i++){
								    	 vo = ovos.get(i);
								     %>
								    	 
								      <label for="mid">아이디 : <%=vo.getMid() %></label><br/> 
								      <label for="nickname">닉네임 : <%=vo.getNickname() %></label>
								    </div>
								    <div class="form-group bGrid">
								      <label for="name">성명  : <%=vo.getName() %></label><br/>
								      <label for="genter">성별 : <%=vo.getGender() %></label><br/>
								      <label for="birthday">생일 : <%=vo.getBirthday().substring(0, 10) %></label><br/>
								      <label for="tel">전화번호 : <%=vo.getTel() %></label>
								    </div>
								    <div class="form-group bGrid">
								      <label for="address">주소 : <%=vo.getAddress() %></label><br/>
								      <label for="email">Email address : <%=vo.getEmail() %></label><br/>
									    <label for="homepage">Homepage address : <%=vo.getHomepage() %></label>
									  </div>
								    <div class="form-group bGrid">
								      <label for="name">직업 : <%=vo.getJob() %></label><br/>
								  	  <label class="form-check-label">취미 : <%=vo.getHobby() %></label><br/>
										  <label class="form-check-label">정보공개 : <%=vo.getUserinfor() %></label>
								    </div>
								    <div class="form-group">
										  <label class="form-check-label">방문횟수 : <%=vo.getVisitCnt() %></label><br/>
								    </div>
								    <div class="form-group bGrid">
										  <label class="form-check-label">최최 가입일자 : <%=vo.getStartDate() %></label><br/>
										  <label class="form-check-label">마지막 접속일자 : <%=vo.getLastDate() %></label>
								    </div>
								 <%}%>   
