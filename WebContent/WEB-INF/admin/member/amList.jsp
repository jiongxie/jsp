...
<div class="background-image-container white-text-container" style="background-image: url('./assets/images/img-13.jpg')">
    ...
    <div class="container">
		    <div class="dropdown">
		    	...
		    	ArrayList<MemberVo> mvos = (ArrayList<MemberVo>) request.getAttribute("mvos");
		    ...
		    <div class="dropdown">
			    ...
			    <a href='${contextPath }/components.jsp'><button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown"  style="color:black;" >
					...
					<h1>운용금액<a style="color:red;"><%=Fund %></h1>
        <div class="row">
            <div class="col-xs-12">
									  <table>
									    <tr>
									...
									%>
									      <tr>
									        ...
									        <td><%=vo.getMoney()%></td>
									        <td>
									          <a href="<%=request.getContextPath()%>/memberView.ad?mid=<%=vo.getMid()%>" target="_blank"><font color='blue';><%=vo.getMid()%></font></a>
									        </td>
									        ...
									        <td>
									        	<a href="javascript:memberResetCheck('<%=vo.getMid()%>')"><font color="blue">강제탈퇴</font></a><br/>
									          <%=vo.getStartDate().substring(0, 10) %>
									        </td>
									      </tr>
									<%
									      curScrNo--;
									...
									%>    
									  ...
									  <div style="text-align:center;">
									    <% if(pag != 1) { %>
									       [<a href="amList.ad?pag=1" style="color:black;">첫페이지</a>]
									    <% } %>
									    <% if(pag > 1) { %>
									       [<a href="amList.ad?pag=<%=pag-1%>" style="color:black;">이전페이지</a>]
									    <% } %>
									    <%=pag%>Page / <%=totPage%>Pages 
									    <% if(pag < totPage) { %>
									       [<a href="amList.ad?pag=<%=pag+1%>" style="color:black;">다음페이지</a>]
									    <% } %>
									    <% if(pag != totPage) { %>
									       [<a href="amList.ad?pag=<%=totPage%>" style="color:black;">끝페이지</a>]
									    <% } %>
									  </div>
									<!-- 페이징 처리 끝   -->  
									</div>
