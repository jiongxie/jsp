...
<head>
  ...
  <script>
    ...
    function delCheck() {
    	...
    	if(ans) {
    		location.href="${contextPath}/schedule/scDelect.sc?ymd=${ymd}&mid=${mid}";
    	}
    }
  </script>
</head>
  ...
  <form name="myform" method="post" action="${contextPath}/scContentOk.sc">
    ...
    <div class="form-group">
      ...
      <select class="form-control" id="memory" name="memory" size="1">
        ...
        <option value="설명회" ${vo.memory=="설명회" ? "selected" : ""}>설명회</option>
    ...
    <div class="form-group">
     ...
     <c:if test="${(empty vo.title) and (smid eq 'admin')}">
     	<button type="button" class="btn btn-secondary" onclick="fCheck()">일정올리기</button>
     </c:if>
     <c:if test="${(!empty vo.title) && (smid =='admin')}">
     	<button type="button" class="btn btn-secondary" onclick="fCheck()">일정수정하기</button>
     	<button type="reset" class="btn btn-secondary">다시쓰기</button>
       <button type="button" class="btn btn-secondary" onclick="delCheck()">일정삭제</button>
     </c:if>
   	<button type="button" class="btn btn-secondary" onclick="location.href='${contextPath}/schedule.sc?ymd=${ymd}&mid=${mid}';">돌아가기</button>
    </div>
